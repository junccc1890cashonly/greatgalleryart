...
import { DAILY_INSPIRATION_COLLECTION_ID, getDailyInspirationPayload } from "./daily-inspiration.js";

const STATE_PREFIX = "state/gallery-state-";
...
function sanitizeCollection(collection) {
  return {
    ...
    dailyDateKey: String(collection.dailyDateKey || ""),
    dailyTitle: String(collection.dailyTitle || ""),
    dailyMood: String(collection.dailyMood || ""),
    dailyPrompt: String(collection.dailyPrompt || ""),
    dailyPalette: Array.isArray(collection.dailyPalette) ? collection.dailyPalette.map((item) => String(item)) : []
  };
}
...
function ensureDailyInspirationState(state, date = new Date()) {
  const sourcePhotos = (state.photos || []).filter((photo) => photo.collectionId !== DAILY_INSPIRATION_COLLECTION_ID);
  const payload = getDailyInspirationPayload(date, sourcePhotos);
  const existingCollection = (state.collections || []).find((collection) => collection.id === DAILY_INSPIRATION_COLLECTION_ID);
  const existingPhotos = (state.photos || []).filter((photo) => photo.collectionId === DAILY_INSPIRATION_COLLECTION_ID);
  const expectedPhotoIds = payload.images.map((_, index) => `${DAILY_INSPIRATION_COLLECTION_ID}-${payload.dateKey}-${index + 1}`);

  const collectionNeedsUpdate =
    !existingCollection ||
    existingCollection.dailyDateKey !== payload.dateKey ||
    existingCollection.dailyMood !== payload.mood ||
    String(existingCollection.description || "") !== payload.description;

  const photosNeedUpdate =
    existingPhotos.length !== payload.images.length ||
    existingPhotos.some((photo, index) => photo.id !== expectedPhotoIds[index] || photo.image !== payload.images[index]);

  if (!collectionNeedsUpdate && !photosNeedUpdate) {
    return { state, changed: false };
  }

  const dailyCollection = sanitizeCollection({
    id: DAILY_INSPIRATION_COLLECTION_ID,
    name: "Daily Inspiration",
    description: payload.description,
    tags: payload.tagSet,
    updatedAt: formatShortDate(date),
    dailyDateKey: payload.dateKey,
    dailyTitle: payload.title,
    dailyMood: payload.mood,
    dailyPrompt: payload.prompt,
    dailyPalette: payload.palette
  });

  const dailyPhotos = payload.images.map((image, index) =>
    sanitizePhoto({
      id: expectedPhotoIds[index],
      title: `${payload.title} Reference ${String(index + 1).padStart(2, "0")}`,
      note: payload.prompt,
      collectionId: DAILY_INSPIRATION_COLLECTION_ID,
      tags: payload.tagSet,
      image,
  ...
  const otherCollections = (state.collections || []).filter((collection) => collection.id !== DAILY_INSPIRATION_COLLECTION_ID);
  const otherPhotos = (state.photos || []).filter((photo) => photo.collectionId !== DAILY_INSPIRATION_COLLECTION_ID);

  return {
    changed: true,
    state: {
      collections: [dailyCollection, ...otherCollections],
      photos: [...dailyPhotos, ...otherPhotos]
    }
  };
}
...
async function persistState(state) {
  const pathname = `${STATE_PREFIX}${Date.now()}.json`;
  await put(pathname, JSON.stringify(normalizeState(state), null, 2), {
    access: "public",
...
export async function getGalleryState() {
  ...
  const ensured = ensureDailyInspirationState(state);
  if (ensured.changed) {
    await persistState(ensured.state);
  }
  return ensured.state;
}

export async function updateGalleryState(updater) {
  const { state } = await readStoredState();
  const ensuredState = ensureDailyInspirationState(state).state;
  const nextState = normalizeState(await updater(ensuredState));
  const finalState = ensureDailyInspirationState(nextState).state;
